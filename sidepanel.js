...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const btnCheckIpInfo = document.getElementById('btnCheckIpInfo');
  ...
  btnCheckIpInfo?.addEventListener('click', () => {
    qaGovernanceMenu?.classList.add('hidden');
  ...
  document.getElementById('btnVideoSettings')?.addEventListener('click', () => {
    ...
    openQaWorkspace('Video Recording', 'Settings');
    // Always fetch fresh settings from background
    chrome.runtime.sendMessage({ action: 'GET_STATE' }, (res) => {
      const settings = res?.data?.videoSettings || currentState.videoSettings || { uploadUrl: '', apiKey: '' };
      qaWorkspaceBody.innerHTML = `
        <div class="qa-entity-form">
      ...
      `;
      qaWorkspaceBody.querySelector('#btnSaveVideoSettings')?.addEventListener('click', () => {
        const payload = {
          autoRecord: isPressed(recordVideoCheck),
        ...
        chrome.runtime.sendMessage({ action: 'SAVE_VIDEO_SETTINGS', payload }, () => {
          currentState.videoSettings = { ...payload };
        });
        closeQaWorkspace();
        showBentoAlert('Tersimpan', 'Pengaturan video berhasil disimpan.', '✅');
      });
    });
  });

  // ==========================================
  // AI SETTINGS UI (inside closure)
  // ==========================================
  document.getElementById('btnAiSettings')?.addEventListener('click', async () => {
    closeQaGovernanceMenu();
    openQaWorkspace('AI Copilot & Services', 'Settings');
    ...
    qaWorkspaceBody.querySelector('#btnSaveAiSettings')?.addEventListener('click', async () => {
      const payload = {
      ...
      currentState.aiSettings = { ...payload };
      
      chrome.runtime.sendMessage({ action: 'SAVE_AI_SETTINGS', payload }, () => {});
      closeQaWorkspace();
      showBentoAlert(tr('Tersimpan'), `${tr('Pengaturan AI')} (${payload.provider} - ${payload.model}) ${tr('Pengaturan AI berhasil disimpan!')}`, '✨');
